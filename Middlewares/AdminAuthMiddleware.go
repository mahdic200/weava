...
func AdminAuthMiddleware(c *fiber.Ctx) error {
	...
	if err := tx.Table("admin_sessions").Where("id = ?", entity_id).Find(&session).Error; err != nil {
	...
	if err := tx.Table("admins").Where("deleted_at IS NULL").Where("id = ?", session.Admin_id).Find(&admin).Error; err != nil {
		return c.Status(500).JSON(fiber.Map{
			"message": Providers.ErrorProvider(err),
		})
	}
